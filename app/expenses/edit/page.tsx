...
function EditExpenseForm() {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, 'expenses', id)).then((snap) => {
      ...
      if (data.splits && Object.keys(data.splits).length > 0) {
        ...
        for (const [m, v] of Object.entries(data.splits)) {
          cs[m as Member] = String(v)
        }
        setCustomSplits(cs)
      } else {
        setSplitMode('equal')
        setSplitAmong((data.splitAmong ?? ['YY', 'Wei', 'Rae']) as Member[])
      }
      setLoaded(true)
    })
  }, [id])
  ...
  const handleSubmit = async () => {
    if (!totalAmount || !item.trim() || submitting) return
    setSubmitting(true)
    ...
    if (splitMode === 'custom') {
      ...
      payload = {
        ...
        splitAmong: Object.keys(splits), splits,
      }
    } else {
      payload = {
  ...
  const perPerson = splitMode === 'equal' && splitAmong.length > 0
    ? Math.round(totalAmount / splitAmong.length) : 0

  if (loadError) return (
    <div className="px-5 pt-20 text-center">
      <p className="text-gray-500 text-sm">找不到此費用</p>
      <Link href="/expenses" className="text-rose-500 text-sm mt-2 inline-block">返回拆帳</Link>
    </div>
  )
  if (!loaded) return <div className="px-5 pt-20 text-center text-sm text-gray-400">載入中...</div>
    ...
    <div>
      ...
      <div className="px-4 space-y-5">
        ...
        <button
          ...
          disabled={
            !totalAmount || !item.trim() || submitting ||
